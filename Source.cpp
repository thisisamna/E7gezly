...
void addTime(string timeSlots[][TIME_NUM], int docIndex, int lastDocTime)
{
	...
	do {
		...
		cin >> choice;
	} while ((choice == 'y' || choice == 'Y') && (lastDocTime < TIME_NUM));
	displayTimeSlots(docIndex, infoDocs[docIndex].docLastTime);
...
void removeTime(string timeSlots[][TIME_NUM], int docIndex, int lastDocTime) //F7
{
	...
	displayTimeSlots(docIndex, infoDocs[docIndex].docLastTime); // BEFORE
	int selection;
	cin >> selection;
	for (int i = selection - 1; i <= TIME_NUM; i++)
	{
		timeSlots[docIndex][i] = timeSlots[docIndex][i + 1];
	}
	lastDocTime--;
	cout << "After: \n";
	displayTimeSlots(docIndex, infoDocs[docIndex].docLastTime); //AFTER
}
void editTime(string timeSlots[][TIME_NUM], int docIndex)
{//here
	...
	string time;
	time = startTime + " - " + endTime;
	timeSlots[docIndex][selection-1] = time;
